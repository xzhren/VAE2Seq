...
def wasserstein_loss(source_dist, target_dist):
    ...
    feature_num_tensor = tf.cast(tf.range(feature_num) / feature_num, tf.float32)# c
    ...
    wasserstein_dist = tf.reduce_sum( (source_index_dist - target_index_dist) * (source_dist - target_dist) , [1,2])
    wasserstein_dist = wasserstein_dist / tf.reduce_sum( (source_dist - target_dist), [1,2]) # b

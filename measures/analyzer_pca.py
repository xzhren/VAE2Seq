...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        for _ in tqdm(range((test_len-1)//args.batch_size+1)):
            ...
            # print("-----------------")
            predict_z = model.encoder_model.get_z_by_sent(sess, x_enc_inp)
            # print(predict_z.shape)
            predict_z_lt.append(predict_z)
            # print("-----------------")
        ...
        predict_z = np.vstack(predict_z_lt)
        print("-----------------")
        print(predict_z.shape)
        print("-----------------")
        ...
        projected = pca.fit_transform(predict_z)

        plt.scatter(projected[:, 0], projected[:, 1], edgecolor='none',
                    alpha=0.5, cmap=plt.cm.get_cmap('Spectral', 10))
        plt.xlabel('component 1')
        plt.ylabel('component 2')
        # plt.colorbar()
        plt.savefig("logs/"+"pca.pdf", bbox_inches='tight')

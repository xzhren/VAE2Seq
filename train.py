...
def main():
    ...
    for epoch in range(args.num_epoch):
        ...
        for i, (enc_inp, dec_inp, dec_out) in enumerate(dataloader.next_batch()):
            # idx2word = dataloader.idx2word
            # print(i, "enc_inp:", len(enc_inp[-1]), ' '.join([idx2word[idx] for idx in enc_inp[-1]]))
            # print(i, "dec_inp:", len(dec_inp[-1]), ' '.join([idx2word[idx] for idx in dec_inp[-1]]))
            # print(i, "dec_out:", len(dec_out[-1]), ' '.join([idx2word[idx] for idx in dec_out[-1]]))
